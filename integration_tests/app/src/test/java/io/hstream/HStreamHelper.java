...
public class HStreamHelper {
    ...
    void createConnector(String name, String sql) {
        ...
        var res = getStub(lookupRes.getServerNode().getPort())
    ...
    HStreamApiGrpc.HStreamApiBlockingStub getStub() {
        if (channels.isEmpty()) {
            channels.put(service.getServerPort(), ManagedChannelBuilder.forAddress(serverHost, service.getServerPort()).usePlaintext().build());
        }
